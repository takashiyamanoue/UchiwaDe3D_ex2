...
{
	private static String TAG="ReadS";
    ...
    public LispObject readProgram(CQueue i)
    {
        in=i;
        ...
        while(!in.isEmpty()){
           if(readCommentLine()) {}
             ...
             String lx=this.lisp.print.print(line);
             Log.d(TAG,"readProgram line="+lx);
    ...
    public LispObject readLine(CQueue i)
    {
        in=i;
...
//        if(!(isNumber(c)||isLetter(c)||isBreak(""+c))) return true;
        if(c<0) {
//        	Log.d(TAG,"isEOL, c<0");
        	return true;
        }
